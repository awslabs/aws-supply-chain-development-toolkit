...
def _validate_type_casting(df: DataFrame, entity: Entity) -> List[Record]:
    ...
    for field in [field for field in entity.fields if field.name in df.columns]:
        if field.type == EntityFieldType.int or field.type == EntityFieldType.double:
            sample_df = df[~df[field.name].isna()]
            invalid_df = sample_df[pd.to_numeric(sample_df[field.name], errors="coerce").isna()]
            if invalid_df.shape[0] > 0 and sample_df.shape[0] > 0:
                records.append(
                    Record(
            ...
            if invalid_df.shape[0] > 0:
                records.append(
                    Record(
                        status=Status.WARNING,
                        entity=entity.displayName,
                        description=f"Field '{field.name}' has values that do not typecast to a date, update to format YYYY-MM-DD",
                        sample_data=invalid_df.head(5),
                    )
                )
